...
if "x%CRoot%x"=="xx" FOR /F "skip=2 tokens=2*" %%A IN ('REG QUERY "%CRegKeyPath%" /v rootdir') DO SET CRoot=%%B
if "x%CRoot%x"=="xx" FOR /F "skip=2 tokens=2*" %%A IN ('REG QUERY "%CRegKeyPath64%" /v rootdir') DO SET CRoot=%%B

:: H options
if "x%HRoot%x"=="xx" FOR /F "skip=2 tokens=2*" %%A IN ('REG QUERY "%HRegKeyPath%" /v InstallPath') DO SET HRoot=%%B
if "x%HRoot%x"=="xx" FOR /F "skip=2 tokens=2*" %%A IN ('REG QUERY "%HRegKeyPath64%" /v InstallPath') DO SET HRoot=%%B

:: R options
if "x%RRoot%x"=="xx" FOR /F "skip=2 tokens=2*" %%A IN ('REG QUERY "%RRegKeyPath%" /v InstallPath') DO SET RRoot=%%B
